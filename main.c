...
int build_tree(const Index *idx, const char *prefix, uint8_t *out_sha)
{
	...
	for (int i = 0; i < idx->count && t.count < MAX_TREE_ENTRIES; i++) {
		...
		if (e->stage != 0)
			continue; /* 충돌 엔트리 제외 */
		if (plen && strncmp(e->path, prefix, plen) != 0)
			continue;
		...
		if (!slash) {
			...
			for (int k = 0; k < t.count; k++)
				if (t.entries[k].mode == MODE_DIR &&
				    ...
				    strncmp(t.entries[k].name, rest, dlen) == 0) {
					dup = 1;
					break;
				}
			if (dup)
			...
			snprintf(sub_prefix, sizeof(sub_prefix), "%.*s%.*s/", (int)plen, prefix,
			         (int)dlen, rest);

			TreeEntry *te = &t.entries[t.count++];
			te->mode = MODE_DIR;
			snprintf(te->name, sizeof(te->name), "%.*s", (int)dlen, rest);
			if (build_tree(idx, sub_prefix, te->sha1) < 0)
				return -1;
		}
	}
...
void checkout_tree(const char *tree_hex, const char *prefix)
{
	...
	for (int i = 0; i < t.count; i++) {
		...
		if (e->mode == MODE_DIR) {
			...
			uint8_t *data = NULL;
			size_t data_len = 0;
			if (object_read(sha1_hex, type, &data, &data_len) < 0)
				continue;
...
int index_from_tree(Index *idx, const char *tree_hex, const char *prefix)
{
	...
	for (int i = 0; i < t.count; i++) {
		...
		if (e->mode == MODE_DIR) {
			...
			if (index_from_tree(idx, sha1_hex, sub) < 0)
				return -1;
		} else {
...
{
	if (argc < 2) {
		fprintf(stderr, "usage: jg <command>\n");
		return 1;
	}
	...
	if (strcmp(cmd, "init") == 0)
		...
		return cmd_commit_tree(argc - 2, argv + 2);
